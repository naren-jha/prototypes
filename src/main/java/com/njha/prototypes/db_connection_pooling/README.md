...
- By using blocking queue for connection pool implementation and haveing a predefined fixed size of the pool, it makes sure that the application will never initiate more than 'x' number of connections with the database (where 'x' is an application config), thereby avoiding database connection overload issues.
